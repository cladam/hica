...
## Step 7: The `actor` keyword lands (13 August 2026)

**Milestone**: M6 of the effects roadmap
([`documentation/effects-design.md`](../../documentation/effects-design.md) §11.4,
[`documentation/effects-journal.md`](../../documentation/effects-journal.md#milestone-6--actor-keyword-desugar)).

The `actor` keyword no longer produces a Phase-1 struct + `<name>_receive` pair.
It now desugars to a real algebraic effect handler:
...
```hica
effect Counter {
  fun send_counter(msg: CounterMsg) : ()
}
...
### What changed vs Steps 1–6

- __State + dispatch → one `handle` block.__ Step 1's `with-counter(init, action)` pattern is now generated automatically. The user never writes `process_messages` again.
- __Cross-actor communication → nested `with_<name>` calls.__ Step 5's two named-effect instances become two `actor` declarations, and the ping-pong rally happens inside `with_pinger(() => with_ponger(() => rally(3)))`.
- __Naming collision workaround.__ The design doc §13.4 anticipated the flat op-namespace issue. The compiler resolves it by naming each actor's send op `send_<name>` (e.g. `send_pinger`, `send_ponger`) so two actors in the same file never collide. When named effects land (post-M6, §10) the surface will collapse to `pinger.send(Pong)`.

### Findings — what the pattern still cannot express

Step 6's mental-process translation flagged three limitations. Two of them are now resolved; one persists and gets a fresh workaround.

1. __✓ Multi-actor orchestration.__ Step 6 could not spawn two actors in one function because `process_messages` was pinned to the first message type. With `actor` sugar each helper is monomorphic, so `with_pinger(() => with_ponger(() => …))` works out of the box. See `examples/effects/ping-pong-actor.hc`.
2. __✓ Return-value leaking.__ Step 6's `weekend_receive` returned `WeekendState` because the receive function still had to hand state back. The desugared handler now returns `()` from every arm (the design-doc §5.3 auto-resume), and state lives inside `with var` for the handler's lifetime. Nothing leaks to `main`.
3. __✗ Observing state from outside the handler.__ Op arms are unit-typed, so if `main` needs the final `count` after `with_counter` returns, it still has to capture into an outer `var`. `learn/47-effects-actors.hc` documents this pattern explicitly; the design-doc solution (a `return-with-state` shape or a dedicated `get` op) is a v2 topic.
4. __✗ Non-resuming control effects.__ Step 6's `ctl step-away()` (never resumes) still has no equivalent in the actor surface. Design doc §10 defers non-resuming handlers to a post-M6 milestone.

### Design implications carried into the ecosystem

- Actors sit *on top of* effects. Every constraint of the effect layer (M1–M5.5) applies transitively: exhaustiveness, effect-row polymorphism, stateful handlers, match-in-resume hoisting. This means the actor sugar automatically inherits, for example, the guarantee that unhandled messages are a *compile* error via M2's exhaustiveness check.
- `with_<name>` helpers cross module boundaries just like ordinary `pub fun`. An actor declared in `lib/bank.hc` becomes callable from `main.hc` via `import "../lib/bank"` and `with_bank(…)` — no separate runtime, no wiring code.
- The Phase-1 `process_messages` examples (`examples/counter-actor.hc`, `examples/ping-pong-actor.hc`, `examples/bank-actor.hc`) still work because they never used the `actor` keyword — they were plain `fun`-based reference implementations. They now stand as "how you would write this without the sugar" companions to their `examples/effects/*.hc` M6 counterparts.

### The proposed hica syntax now maps 1:1 to Koka

The old mapping table from Step 5 becomes:

| hica syntax | Koka codegen | |---|---| | `actor Counter { var count = 0; receive(m: CounterMsg) => … }` | `effect counter { ctl hc_send_counter(msg : countermsg) : () }` + `pub fun hc_with_counter(action) { var count := 0; with handler ctl hc_send_counter(msg) -> … ; action() }` | | `with_counter(() => { … })` | `hc_with_counter(fn() { … })` | | `send_counter(Incr)` | `hc_send_counter(Incr)` |

The `spawn` / `send` / `ask` runtime primitives from `actors-ideation.md` are no longer needed as builtins — they're just `with_<name>` and `send_<name>` generated by the compiler.

### Next steps beyond M6

- __Named effects__ (design doc §10) — the natural v2 step. Would let two actors share the op name `send` cleanly (`pinger.send(Pong)` / `ponger.send(Ping)`) and enable spawning multiple independent instances of the same actor type (`spawn(Counter)` returning a fresh handle).
- __Actor supervision__ — an outer handler that catches errors from an inner actor and either restarts or escalates. Trivial to prototype on top of `effect Supervisor { fun on_crash(reason: string) }` + `with_supervisor` — no language changes needed.
- __`?` in receive bodies__ — the `hica-early-*` effect and the actor's send op compose naturally in Koka. Once we have a concrete case that wants "an actor message that can early-return", we'll wire it up.
