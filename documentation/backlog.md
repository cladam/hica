...
| Overload `length` to work on strings | **done** | Low | `length(s: string)` should return `str_length(s)`. Currently `length` is list-only and `length(s)` on a string produces a confusing type error (`expected list<t>, got string`). Every beginner writes `length(s)`. Fix: add a second extern sig for `TString` in the checker, or desugar `length` calls on strings to `str_length` in the type checker. Python, Swift, and Kotlin all unify collection length under one name. Priority: high — discovered as root cause of all cursor-based parser failures in hica-assistant testing. |
| Overload `is_empty` to work on lists | **done** | Low | `is_empty([]) : bool` should return true. Currently `is_empty` takes only `string` and `is_empty(xs)` on a `list<T>` is a type error. Fix: add a second overload `is_empty(xs: list<T>) : bool` as a prelude alias for `xs == []`, or desugar in checker. Beginner-facing: every learner who checks for an empty list will try `is_empty` first. |
...
| Struct spread update `{ ...s, f: v }` | **done** | Medium | Allow partial struct updates: `Point { ...p, x: 5 }` copies all fields from `p` then overrides `x`. Parser supports `...expr` inside struct literal; checker validates the base expression is the same struct type; codegen expands to a full constructor call reading unmentioned fields. Currently users must reconstruct the struct fully. High usability impact whenever models evolve. Prerequisite for practical optics. Inspired by Arrow Optics' immutable data story. |
| `Validated` type (accumulating errors) | **done** | Medium | A sum type `type Validated { Valid(value), Invalid(errors) }` that accumulates ALL errors instead of short-circuiting on the first. Unlike `result<T,E>` with `?`, a `Validated` pipeline collects every failure before returning. Primary use case: form/config validation that reports all problems at once. `zip_validated(v1, v2, f)` runs both validations and combines results or merges error lists. Can be implemented as a library (no language changes needed) but benefits from `&?` syntax sugar. Inspired by Arrow's `Validated`/`zipOrAccumulate`. |
| `&?` operator (validated accumulation) | **done** | High | Postfix `&?` on `Validated<T>` — accumulates errors across multiple validation steps instead of short-circuiting. Mirrors what `?` does for `Result`. Requires new AST node, lexer token, checker inference, and Koka effect codegen using `hica-validated` effect. Dependent on `Validated` type being available as a stdlib type. Inspired by Arrow's `zipOrAccumulate` DSL. |
...
| Numeric domain primitives (`Quantity`, `Price`, …) | **—** | Low | Document and optionally provide a `std/domain` module with patterns for opaque integer types: `Quantity` (positive, bounded), `Price` (non-negative cents), `RoomNumber`. Prevents negative-quantity discount exploits, argument-order confusion, and semantically meaningless arithmetic (bus number + bus number). Pattern is: `opaque struct Foo { n: int }` + smart constructor that validates range. See `examples/domain-primitives.hc` for working reference. Backlog until demand warrants a stdlib module. |
| User-facing algebraic effect definitions (`effect` / `handle`) | **done** | High | Delivered in milestones M1–M3. `effect Name { fun op(...) : T ... }` declares an effect; `handle Name { arms } in { body }` installs a handler; the checker validates op arms exhaustively (missing/unknown/duplicate ops) and reports them with source spans. Op arm parameters get types from the effect declaration automatically. `hica check` reports user-defined effects in the effect row alongside built-ins (`<console, Log, Db>`). Full docs at [`docs/effects.md`](../docs/effects.md); design at [`documentation/effects-design.md`](effects-design.md). See `examples/effects/hello-effect.hc`, `learn/44-effects-intro.hc`. |
| Effect-row polymorphic function types | **done** | High | Delivered in milestone M4. Function types accept an optional effect row: `pub fun with_db(f: () -> <Db> int) : int`. An empty (or absent) row is a wildcard; a non-empty row is authoritative and enforced at every call site — passing a callback that leaks an effect not in the row is a compile-time error (`error: effect row mismatch — callback passed to 'with_db' may use <Log, Db>, not permitted by <Db>`). Enforcement follows callback references to top-level decls (X1 refinement). Unblocks the capability-based-security pattern from [`documentation/security-type-boundaries.md`](security-type-boundaries.md) §7. See `examples/effects/db-sandbox.hc` (positive), `examples/effects/db-sandbox-leak.hc` (negative), `learn/46-effects-sandbox.hc`. M4.5 codegen-collision fix tags user effect-op call sites so ops named `exec`/`write`/`print`/etc. no longer collide with hica's stdlib codegen intercepts. |
| Stateful handlers (`with var …`) | **done** | Medium | Delivered in milestone M5 (with the M5.5 match-in-resume codegen follow-up). `handle E { arms } with var x = init, var y = init in { … }` hoists mutable state before the handler installation; every arm body and the `in` block share the same references, and the state dies when the block returns. Codegen splits assignment-shaped arms so the mutation runs *before* the auto-`resume(())`, and match-shaped arms are hoisted to a local `val` so Koka's layout parser accepts them. See `examples/effects/counter.hc`, `examples/effects/buffer.hc`, `learn/45-effects-state.hc`. |
| `actor` keyword (sugar over `effect + handle + with var`) | **done** | Medium | Delivered in milestone M6. `actor Name { var s = init; receive(msg: T) => body }` desugars into an `effect Name { fun send_<name>(msg: T) : () }` declaration plus a `pub fun with_<name>(action) { handle Name { send_<name>(msg) => body } with var s = init in { action() } }` helper. Each actor gets its own `send_<name>` op — with the current flat effect-op namespace this sidesteps two-actor collisions (see design doc §8.2, §13.4; named effects will collapse the naming back to `actor.send(m)` post-M6). The `msg` parameter on `receive` must carry an explicit type annotation. See `examples/effects/counter-actor.hc`, `examples/effects/ping-pong-actor.hc`, `learn/47-effects-actors.hc`. |
| Named effects v2 — parser + codegen (N1) | **done** | High | Delivered in milestone N1. `spawn Name { arms } (with var …)? as ident` parses and emits Koka's `with <binder> <- named handler` form with `fun`-shaped tail-resumptive ops. Effects that appear in any `spawn` are promoted to `named effect …` at emission (design doc §7.6). `emit-type(TRef(name))` renders as `ev<name>`. Per-instance method dispatch (`c1.incr()`) is deferred to N2. See `examples/effects/two-counters.hc`, `tests/test-named-effects.kk`, [`documentation/named-effects-design.md`](named-effects-design.md), [`documentation/named-effects-journal.md`](named-effects-journal.md). |
| Named effects v2 — method dispatch + effect promotion checker (N2) | **done** | High | Delivered in milestone N2. `Call(Var(op), [ref, args...])` where `ref : TRef(E)` and `op ∈ E.ops` is retagged as `Var("hc-ref-op:<op>")` by the checker; codegen emits `ref.hc_<op>(args)` — Koka's named-handler dispatch shape. Op signatures are freshened per call and unified against the arg list, so type-mismatch errors carry source spans. The test-program emit path (`emit-test-program`) was patched to use the same spawn-promotion set as `emit-program`. See `examples/effects/two-counters.hc` (now runs `c1 = 3` / `c2 = 101`), `learn/48-named-effects.hc`, `tests/test-named-effects.kk` (10 cases). |
| Named effects v2 — escape rule + `ref<E>` in signatures (N3) | **done** | High | Delivered in milestone N3. `parse-type` accepts `ref<Name>` alongside `list`, `maybe`, `stream`, `result`. Unification treats `TRef(A)` nominally. Codegen appends `div,exn` to every explicit function effect row so declared rows like `<Counter>` compile against Koka's baseline. Escape rule: `check-ref-escapes` walks each decl body, collects locally-spawned `ESpawn` binders, and errors on any return-position `Var(r)` that names one — parameter refs are treated as caller-owned and skipped. See `examples/effects/counter-pool.hc`, `tests/test-named-effects.kk` (13 cases). Known limitation: inline `.op()` inside untyped `foreach` lambdas — declare a `ref<E>`-typed helper instead. |
| Named effects v2 — `hica check` row reporting + analyser (N4) | **done** | Medium | Delivered in milestone N4. `hica check` reports one row entry per effect regardless of dispatch style — spawn-only, mixed handle+spawn, and cross-effect scenarios all surface exactly one row entry per effect. Turned out to be a no-code milestone: `collect-calls-h`'s ESpawn arm (N1) plus `unwrap-op-tag-main`'s `hc-ref-op:` stripping (N2) already handled every case correctly. Milestone landed as tests-only + a `pub fun discover-effects` visibility change so tests can call it from `main`. Five new regression tests: three in `tests/test-named-effects.kk` (spawn-only, two-spawn cross-effect, handle+spawn on same effect appears once), two in `tests/test-analyser.kk` (walker doesn't crash on ESpawn; bare spawn scaffolding surfaces no false debt). Makefile: `test-named-effects` now links libcurl transitively (needed once we import `main`). |
| Named effects v2 — `actor` sugar retires `send_<name>` (N5) | **done** | Medium | Delivered in milestone N5. `actor Name { … }` now desugars to a single `effect Name { fun send(msg) : () }` — no `send_<name>` suffix, no `with_<name>` callback helper. Users install instances directly with `spawn Name { send(msg) => body } as ref` and dispatch with `ref.send(msg)`. Two actors both declaring `send` no longer collide because codegen emits effect-qualified op names (`hc_<effect>_<op>`) for every `named effect` — avoiding Koka's `hc_<op>/@select` selector clash. Bonus checker fix: `extend-let-chain` now threads `ESpawn` binders through Let/VarDecl bodies, so a spawn nested after a `var … = -1` at the top of a function is still visible to subsequent statements. See `examples/effects/ping-pong-actor.hc`, `examples/effects/counter-actor.hc`, `learn/47-effects-actors.hc`, and `tests/test-named-effects.kk` (+ N5 test case in `tests/test-effects.kk`). |
...
| Targeted error for `..` inside slice position | **done** | Low | When the parser sees `s[expr..expr]`, it errors with `unexpected token: ]` which is confusing — the user sees the wrong token, not the real problem. Fix: detect `..` inside `[...]` subscript context and emit a specific message: *"slice separator must be a colon, not `..` — write `s[n:m]`, not `s[n..m]`"*. The `..` token is only valid in list spread patterns `[x, ..rest]`. High beginner impact — discovered as a root cause in hica-assistant testing. |
...
| Explicit return type annotation on functions using `while` | **done** | Low | A function like `fun f(c: Cursor) : Cursor { while ... }` causes a Koka compile error: `effects do not match: inferred <div\|_e>, expected total`. The user wrote a legal hica annotation but the generated Koka rejects it. Fix options: (a) detect `While` in function body during codegen and suppress the return type annotation automatically, or (b) emit a hica-level error/warning during type checking: *"return type annotation suppressed: function contains `while` and gets a `div` effect — remove `: ReturnType` or Koka will reject it"*. Option (a) is silent and correct; option (b) is more instructive. |
| `map_result` / `and_then_result` with `match` lambda codegen layout error | **done** | Medium | Writing `r \|> map_result((pair) => match pair { (a, b) => (J(a), b) })` type-checks fine in hica but the generated Koka has a brace-layout bug: the inner `match` arm's `{` interferes with the outer `match hc_parse_x { Right(pair) -> Right(match pair ...` construct. Koka's layout parser closes the outer brace too early. Root cause: the `map_result` codegen wraps the lambda result in `Right(...)`, producing a brace-open that conflicts with the match indentation. Fix: when the argument to `map_result` or `and_then_result` is a lambda whose body is a `Match` expression, IIFE-hoist the result or inline expand as explicit `match r { Right(x) => Right(f(x)), Left(e) => Left(e) }`. Discovered during JSON parser reference implementation — affects any piped map over a parser result with tuple destructuring. |
| `\b` and `\f` in string literals silently produce invalid Koka | **done** | Low | hica passes `\b` (backspace) and `\f` (form feed) through to generated Koka unchanged. Koka only supports `\\`, `\"`, `\n`, `\r`, `\t`, `\0`, `\u{XXXX}` — so the Koka compiler rejects the generated file with "unexpected illegal character in string". From the user's perspective this is silent: they wrote valid-looking hica, it passed `hica check`, and the error appears at Koka compile time with no pointer to the hica source line. Fix: in the lexer or checker, (a) reject `\b`/`\f` with a hica-level error pointing at the exact position in the string literal, or (b) translate to `char_to_string(chr(8))` / `char_to_string(chr(12))` in the string representation. Option (b) is transparent; option (a) at least gives a clear message. Currently the workaround is `char_to_string(chr(8))`. |
| Error messages that reference generated `.kk` line numbers | **done** | Medium | When Koka compilation fails, error messages show `.kk` file paths and line numbers (e.g. `src/parser.kk(248,70): parse error`). The user can't act on these — they're reading `.hc` source. The `.kk` file has `// .hc:37` comments that track the original source line. Fix: intercept Koka error output in the CLI, parse the `.kk` line number, walk backward in the `.kk` file to find the nearest `// .hc:N` comment, and rewrite the error to point at the `.hc` source. This transforms "working one layer above a thing you can't directly edit" into a direct feedback loop. High impact for library authors who hit codegen edge cases. |
...
|---------|--------|------------|-------|
| **nel** | **done** | Low | Non-empty list type. `type Nel { Nel(head, tail) }` — guaranteed at least one element at the type level. Functions: `nel_of(x)`, `nel_to_list(n)`, `nel_map(n, f)`, `nel_head(n)`, `nel_concat(a, b)`. Serves as the error-list type for `Validated` — an `Invalid` can never have zero errors. Pure hica, no language changes required. Inspired by Arrow's `NonEmptyList`. |
| **validated** | **done** | Medium | Error-accumulating validation. `type Validated { Valid(value), Invalid(errors) }` where `errors` is `nel<string>`. Functions: `valid(x)`, `invalid(msg)`, `zip_validated(v1, v2, f)` (run both validations, combine results or merge error lists), `map_validated(v, f)`, `validate_all(validators)`, `as_result(v)`, `from_result(r)`. Unlike `result<T,E>` with `?`, all paths are evaluated so every error surfaces. Primary use case: form/config validation that shows all problems at once. Depends on `nel`. Inspired by Arrow's `Validated`/`zipOrAccumulate`. |
