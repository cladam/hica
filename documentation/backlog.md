...
| `random(min, max)` | **done** | Medium | Random integer in `[min, max)`. Extern backed by Koka `srandom-int32-range`. Adds `ndet` effect. Codegen imports `std/num/random` + `std/num/int32` when used. Learn module: `learn/28-random.hc` |
