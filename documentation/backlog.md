...
| Named effects v2 — method dispatch + effect promotion checker (N2) | **—** | High | Next milestone. Wire `Call(Var(op), [ref, args...])` where `ref : TRef(E)` and `op ∈ E.ops` into a `hc-ref-op:<op>` tagged call that codegen emits as `ref.hc_<op>(args)`. Requires extracting the existing `Call` arm's length/is_empty/stream dispatch into a helper (`infer-call-generic`) so the `if try-ref-dispatch(...) then ... else ...` shell composes cleanly. Emit `error: 'unknown' is not an operation of effect 'E'` with source spans. Wrap `EHandle` on a spawn-promoted effect in `with hc__handle_<N> <- named handler`. |
| Named effects v2 — escape rule + `ref<E>` in signatures (N3) | **—** | High | Milestone N3. Accept `ref<E>` in type annotations (parameters, returns, struct fields, `let`). Enforce §5.5 escape rule: refs can't outlive their handler. Enables `fun bump(c: ref<Counter>, n: int)` and the actor-pool example (`examples/effects/counter-pool.hc`). |
