...
| Named effects v2 — `hica check` row reporting + analyser (N4) | **—** | Medium | Milestone N4. `hica check` reports one row entry per effect regardless of style. Analyser walks `ESpawn` + method calls without false positives. `discover-effects` in `src/main.kk` learns about `spawn`. |
