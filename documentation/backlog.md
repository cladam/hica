...
- **Negative float literal emitted as `(0 - 1.0)` causes Koka int/float64 type error** —
  Writing `amount: -1.0` in a struct literal causes hica codegen to emit `(0 - 1.0)` in
  Koka, where `0` is inferred as `int` and `1.0` is `float64`. Koka rejects the
  subtraction because types don't match. Discovered in `examples/order-workflow-config.hc`
  when passing a negative amount to `Order { ... amount: -1.0 ... }`. **Fix:** codegen
  should emit negative float literals as `-1.0` (unary minus on a float literal) rather
  than `0 - 1.0`. **Workaround:** avoid negative float literals in struct fields; express
  the negative value indirectly (e.g. use `0.0` if the zero boundary is sufficient, or
  compute the value in a `let` binding before the struct literal).
- **Importing a library that uses `is_alpha`/`is_digit` clashes with codegen preamble `hc_is_alnum`** —
  The hica codegen preamble injects `fun hc_is_alnum(c : char)` which calls `hc_is_alpha(c)`
  expecting `c : char`. If a library (e.g. lib/hml) defines its own `pub fun is_alpha(c: string)`
  it shadows the preamble's expected signature, causing a Koka type mismatch: `inferred char,
  expected string`. Discovered when importing `lib/hml/src/hml` in `order-workflow-config.hc`.
  **Fix:** codegen preamble should not call `hc_is_alpha` from `hc_is_alnum`; make `hc_is_alnum`
  self-contained. **Workaround:** rename the library's `is_alpha`/`is_digit` to non-colliding
  names (e.g. `hml_is_alpha`, `hml_is_digit`) before publishing as a submodule library.
