...
| Named effects v2 — `actor` sugar retires `send_<name>` (N5) | **—** | Medium | Milestone N5. `actor` desugar drops the `send_<name>` naming workaround; the ping-pong example collapses from `with_pinger(() => with_ponger(…))` to `spawn Pinger as pinger; spawn Ponger as ponger; rally(pinger, ponger, 3)`. Keep the old shape for one release-cycle backwards-compat window. |
